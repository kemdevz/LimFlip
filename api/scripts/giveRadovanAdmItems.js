...
async function giveRadovanAdmItems() {
  try {
    await mongoose.connect(MONGODB_URI);
    console.log('MongoDB connected successfully');

    // Find RadovanAdm user
    const user = await User.findOne({ username: 'RadovanAdm' });
    if (!user) {
      console.log('❌ User RadovanAdm not found');
      process.exit(1);
    }
    ...
    console.log(`✅ Added ${addedCount} items to RadovanAdm's inventory`);
